...
import openai  # pip install -U openai；把 openai 包匯入程式
...
openai.api_key = os.getenv("OPENAI_API_KEY")  # 建立 OpenAI API 實例
    ...
    try:
        response = openai.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "你是一個專業的氣象預報員，會根據提供的天氣資料，整理出一段簡短的預報摘要，讓一般人也能輕鬆理解未來幾天的天氣狀況。",
                },
                {
                    "role": "user",
                    "content": f"請根據以下的{city}天氣預報資料，幫我整理出一段簡短的預報摘要：\n{raw_forecast}",
                },
            ],
            temperature=0.2,
        )
